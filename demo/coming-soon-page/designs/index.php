<?php
	// TITLE OF PAGE
	$title = "Designs and Their Code";
	
	// STYLING (light or dark)
	$color	= "light";
	
	// ADD SPECIFIC FILES YOU WANT TO IGNORE HERE
	$ignore_file_list = array( ".htaccess", "Thumbs.db", ".DS_Store", "list.php", "error_log" );

	// ADD SPECIFIC FILES YOU WANT TO IGNORE HERE
	$ignore_directory_list = array( "assets", "css", ".DS_Store", "list.php", "error_log" );
...
function build_blocks( $items, $folder )
{
	...
	foreach($objects['directories'] as $c => $file)
	{
		...
		if( $toggle_sub_folders )
		{
			if( $sub_items )
			{
				echo "<a href='#'><div class='sub' data-folder=\"$file\"></a>";
